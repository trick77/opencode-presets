...
| `provider-litellm` | Provider | replace | Point the `litellm` provider at a custom/remote proxy URL for `plugin-litellm-pricing` (prompts for base URL; key via `$LITELLM_API_KEY`, no models list) |
