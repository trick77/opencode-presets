...
| `instructions-swiss-rules` | Instructions | append | Answer in German with Swiss orthography (never the eszett character, always `ss`; umlauts as real characters, never `ae`/`oe`/`ue`), German code comments with German domain nouns in identifiers, and plans written as self-contained HTML to `docs/plans/<TICKET>-<slug>.html` with `<TICKET>` taken from the current branch name (fetches `rules/de-swiss.md` from this repo, sha256-verified) |
